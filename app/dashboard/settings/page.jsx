      ...
      <div className="space-y-3">
        {settingsOptions.map(({ href, label, description, icon: Icon }) => (
          <Link
            key={href}
            href={href}
            className="flex items-center gap-4 bg-white dark:bg-[#111] border border-gray-200 dark:border-gray-800 rounded-2xl p-5 hover:border-gray-300 dark:hover:border-gray-600 hover:shadow-sm transition-all duration-150 group"
          >
            ...
            <ChevronRight className="w-4 h-4 text-gray-400 dark:text-gray-600 shrink-0 group-hover:text-gray-600 dark:group-hover:text-gray-400 transition-colors" />
          </Link>
        ))}
      </div>
    </div>
